...
class Expirations extends Controller {
...
function save_truck()
	...
	$data['truck_id'] = $truckid;
	    ...
	    $_POST['regdate'] = changeDateFromPageToMySQLFormat($_POST['startyear']."-".$_POST['startmonth']."-".$_POST['startday']);
		
		$_POST['endlicedate'] = changeDateFromPageToMySQLFormat($_POST['startyear7']."-".$_POST['startmonth7']."-".$_POST['startday7']);
			$_POST['permit'] = changeDateFromPageToMySQLFormat($_POST['startyear3']."-".$_POST['startmonth3']."-".$_POST['startday3']);
#
if(is_array($_POST['allowedcargo'])){
	...
	#
		...
		if(($this->input->post('saveandnew') || $this->input->post('save')) && $this->process_form_data($urldata, $_POST, 'save'))
		{
			...
			{
				$view_to_load = 'managetruck/expiration';
			}
	...
	{	
		...
		else if($action == 'save')
		{
			...
			if($urldata['truck_id'] !== FALSE)
			{
				$query = $this->Query_reader->get_query_by_code('update_truck_expirations', array_merge(array('truck_id'=>$urldata['truck_id']), $formdata));
			}
