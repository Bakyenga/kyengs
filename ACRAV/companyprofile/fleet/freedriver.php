...
<body>
 ...
 <table width="100%" border="0">
  ...
  <tr>
    <td width="78%" valign="top"><form id="imageform2" name="register_step1" method="post" action="backend.php?free=true" >
      <table width="100%" border="0" cellpadding="5">
          ...
          <tr>
            <td colspan="2" align="left" bgcolor="#FFFFFF" ><span class="heads"> FREE DRIVER FROM A TRUCK </span>
      ...
      <input name="cancel4" type="button" id="cancel4" value="Add Vehicle" onClick="location.href='dashboard.php?p=<?php echo encryptValue("company"); ?>&flag=<?php echo encryptValue("compTruckz"); ?>'" class="button"/></td>
          ...
          <tr>
            ...
            <td width="80%"><input name="companyid" type="hidden" value="<?php if(isset($userdetails['companyid'])){ echo $userdetails['companyid'];} ?>" />
              <input name="truck_id" type="hidden" value="<?php echo $_GET['truck_id']; ?>" />   <input name="assid" type="hidden" value="<?php echo $_SESSION['assdriver']; ?>" /> <?php echo $_SESSION['assdriver']; ?>
              <b>
          ...
          <tr>
            ...
            <td><div id="hidden"><div class="form" style="border: 5px solid #CCCCCC; padding:0px;width:100%;height:450px;overflow: auto" id="searchresults">
            <?php
			$ass='Y';
...
<?php 
  	...
  	$item ="";
			...
			$item ="#F5F5F5";?><tr bgcolor="<?php echo $item;?>" style="">            <td align="center"><label>
  <input type="radio" name="driver_id" id="ass" value="<?php echo $driver['ID']?>" />
  </label></td>
            <td align="left"><a rel="facebox" href="companyprofile/fleet/driver_details.php?&token=<?php echo $driver['ID'];?>&flag=<?php echo $driver['ID']; ?>"><?php echo $driver['Firstname']." ".$driver['Lastname'];?></a></td>
            </tr>
        <td colspan="2" align="left">
        <?php 
				  	
				$i++;  } while($driver = mysql_fetch_array($truckaccidents));
				  } else echo '<br><br>'.'<span class="textdescp">'.'No Drivers to free'.'</span>';
				  ?><br>
        <input name="save" type="submit" class="button" id="save" value="Free Driver"/>     </td>
            </table>
                  </div>
                  </div></td>
          </tr>
