...
<body class="mainbg">
 <table width="100%" border="0" cellspacing="0" cellpadding="10">
      <tr>
        <td colspan="3" align="left" class="heads" bgcolor="#FFFFFF"><b>STEP 6 - MANAGE CARGO</b></td>
...
<form id="" name="register_step1" method="post" action=" backend.php?addcargo=true"  enctype="multipart/form-data" onsubmit='return formValidator()'>
        <table width="100%" border="0" cellspacing="0" cellpadding="10">
              ...
              <tr>
                  		...
                  		<?php 
							if(isset($_GET['container_id'])){ ?>
								<td>Shipment</td><td colspan="3"><a rel="facebox" href="companyprofile/edit-select.php?action=<?php echo encryptValue('3'); ?>&flag=<?php echo encryptValue($scheduledata['ID']); ?>"><?php echo $shipmentedit['SFIDNo'] ?></a>
						<?php }else{ ?>
                        <td colspan="3">
                         <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                              <td height="45"><b>Select Shipment :</b> </td>
                           
                              <td>
                             
                                  <select name="shipmentid" id="shipmentsel" class="textfield" style="width:280px; margin-left:0px;" required>
                                      <option value="" selected>Select the shipment</option>
                                      <?php
                                        $qry = mysql_query("Select * from shipments where CompanyID = '".$_SESSION['UserID']."' and Status = 'Not delivered'");
                                        if(mysql_num_rows($qry) > 0){
                                        $rowq = mysql_fetch_array($qry);
                                        do{
                                          echo "<option value='".$rowq['ID']."'>".$rowq['SFIDNo']."</option>";
...
document.write(test.parseURL());
         ...
         <?php 
          do{ 
			$shipment = mysql_fetch_assoc(mysql_query("SELECT * FROM shipments Where ID = '".$cargo['ShipmentID']."'"));
	if($i%2)
  	$item ="";
		else
			$item ="#F5F5F5";?>
            ...
            <td align="left"><?php echo $shipment['SFIDNo'];?></td>
...
$(document).ready(function(){
  ...
  $("#shipmentsel").change(function()   
  {
    ...
    $("#shipment_details").load("companyprofile/shipmentdetails.php?&randval=" + Math.random()).slideDown("slow");
