...
define ('DIRECTORY_CACHE', './thumbs/cache');		// cache directory
define ('DIRECTORY_TEMP', './thumbs/temp');		// temp directory
define ('MAX_WIDTH', 800);					// maximum image width
define ('MAX_HEIGHT', 600);				// maximum image height
...
$src = get_request ('src', '/images/no_image.jpg');
if ($src == '' || strlen ($src) <= 3) {
    
	display_error ('no image specified');

	//RUTATIINA GILBERT commete the original line and added Lines: 64 t0 70
	//echo $src	=	"/images/no_image.jpg";
 ...
 * @return <type> 
 *
 *	RUTATIINA made a few changes to the function below
 */
...
function show_cache_file ($mime_type) {
    ...
    if (file_exists ($cache_file)) {
		...
		// use browser cache if available to speed up zone load
