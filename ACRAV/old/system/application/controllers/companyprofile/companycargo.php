...
# Displays an empty user form
	...
	# Displays cargo being tracked
	function displayTrackedCargo()
	{
		$data['userdetails'] = $this->session->userdata('alluserdata');
		$id=$data['userdetails']['companyid'];
		$data['id']=$id;
		$query = ('select * from truckCargoTracker where company_id ='.$id);
		$result = $this->db->query($query);
		$data['returned']= $result->num_rows();
		$data['truckcargotracker_array'] = $result->result_array();
		
		$this->load->view('usertracking/manageTrackedCargo', $data);	
	...
	function save_container()
	{
		...
		$urldata = $this->uri->uri_to_assoc(4, array('container_id','action'));
