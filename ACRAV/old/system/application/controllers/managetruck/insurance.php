...
# Displays an empty truck form
	...
	$data['truck_id'] = $truckid;
	    ...
	    $_POST['startdate'] = changeDateFromPageToMySQLFormat($_POST['startyear']."-".$_POST['startmonth']."-".$_POST['startday']);
...
if(is_array($_POST['allowedcargo'])){
	...
	{	
		...
		else if($action == 'save')
		{
			...
			if($urldata['truck_id'] !== FALSE)
			{
				$query = $this->Query_reader->get_query_by_code('update_truck_insurance', array_merge(array('truck_id'=>$urldata['truck_id']), $formdata));
			}
