...
$data['msg'] = "The truck data was successfully saved.";			
	...
	$data['truck_id'] = $truckid;
			if( $truckid !== FALSE)
			{
$data = array('image5' => $_FILES['img1']['name'],'image2'=> $_FILES['img2']['name'],'image3'=> $_FILES['img3']['name'],'image4'=> $_FILES['img4']['name']);
$this->db->where('truck_id', $truckid);
$this->db->update('trucks', $data);


				//$query = $this->Query_reader->get_query_by_code('update_truck_photos', array_merge(array('truck_id'=>$urldata['truck_id']), $formdata));
			}
