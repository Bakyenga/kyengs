...
class Employees extends Controller {
	...
	function save_employee()
	{
		...
		if(($this->input->post('saveandnew') || $this->input->post('save')) && $this->process_form_data($urldata, $_POST, 'save'))
		{
			...
			if(isset($_POST['oldpassword']) && sha1($_POST['oldpassword']) != $old_employee_details['password'] && trim($_POST['password']) != '')
			{
	...
	{	
		...
		else if($action == 'save')
		{
			...
			{	
				...
				if(isset($urldata['user']))
				{
					if(sha1($formdata['oldpassword']) == $employee_details['password'] && trim($formdata['password']) != '')
					{
