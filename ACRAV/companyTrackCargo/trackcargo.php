...
$queryx = "SELECT * FROM truckcargotraker where CompanyID = '".$_SESSION['UserID']."' AND TruckID != '0' ORDER BY ID DESC LIMIT 5000";
...
$queryc = "SELECT * FROM truckcargotraker where CompanyID = '".$_SESSION['UserID']."' AND ContainerID != '0' ORDER BY ID DESC LIMIT 5000";
...
do{
					$container = mysql_fetch_assoc(mysql_query("SELECT m.* FROM containers m WHERE container_id = '". $rowc['ContainerID'] ."' ORDER BY container_id ASC LIMIT 5000"));
					$tracker = mysql_fetch_assoc(mysql_query("SELECT m.* FROM companytrackers m WHERE ID = '". $rowc['TrackerID'] ."' ORDER BY ID ASC LIMIT 5000"));
					$trackerd = mysql_fetch_assoc(mysql_query("SELECT m.* FROM trackers m WHERE ID = '". $tracker['TrackerID'] ."' ORDER BY ID ASC LIMIT 5000"));	
				?>
            <option value="<?php echo encryptValue(ltrim($trackerd['SimNo'],"+"))."*".encryptValue($container['containernumber']); ?>"><?php echo $container['containernumber']; ?></option>
<?php }while ($rowc = mysql_fetch_array($queryc)); }?>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="5" class="">
    ...
    <tr id="vuhist">
       ...
       <table width="100%">
		<?php
			$query = "select * from messages where phone ='+".decryptValue($_GET['gp'])."' order by date_added ASC LIMIT 5000";
...
<b>NOW TRACKING - <span style="color:#820000; font-weight:bold;"><?php echo strtoupper(decryptValue($_GET['wibt'])); ?></span></b>
</td>
                </tr>
                <tr>
  ...
  <tr>
	...
	$rows = mysql_fetch_assoc(mysql_query("select * from messages where phone ='+".decryptValue($_GET['gp'])."' order by date_added DESC LIMIT 5000"));
	?>
